...
行动：**周期 9+ 评估嵌入 cesium-mcp-bridge SDK 到本项目 Viewer**（与 P1-1 mcpManifest.js 集成）。
...
行动：周期 9+ P1：包大小评估 + 兼容性测试；与本项目 P1-1 mcpManifest 集成。
...
行动：**周期 9 P1-3**：memory.js `_initSqlite` 当前未启用 WAL；加 `PRAGMA journal_mode=WAL`（更好并发）。
...
行动：周期 9+ 把这些 pragma 加入 memory.js `_initSqlite`（默认启用）；验证 spec 仍 PASS。
...
行动：**周期 9 P1**：评估把本周期 pr-review.yml 升级为多 specialist 模式（先 baseline + design + security 三 specialist）。
...
行动：**周期 9+ 评估**：用 react-crash-guard 替换本项目 ErrorBoundary.jsx；提供 async error 捕获（本项目目前缺）。
...
行动：周期 9+：在 client/src/utils/errorHandler.js 加全局 unhandledrejection 监听；上报到 /api/telemetry。
...
行动：参考 `/review-pr --post` 模式 —— 本项目周期 8 P1-7 已落 pr-review.yml（baseline + ai-review 占位）；周期 9+ 评估接 claude-code-action。
...
## 调研 Top5（落 upcoming-work，周期 9+ 优先）

| 排名 | 主题 | 行动 | 落点 |
| --- | ---- | ---- | ---- |
| 1 | **MCP 实际接入 cesium-mcp-bridge** | 周期 9+ P1：包大小评估 + 5 个核心工具集成测试 + viewer 集成 | 升级 P1-1 mcpManifest |
| 2 | **mem0 向量化（pgvector）** | 周期 10+ 评估：SQLite → Postgres + pgvector + LLM 抽取层（episodic + semantic + procedural） | 升级 P1-5 mem0 prototype |
| 3 | **memory.js WAL + 性能调优** | 周期 9 P2：加 `PRAGMA journal_mode=WAL` + `synchronous=NORMAL` + 周期 `pragma optimize` | 升级 memory.js |
| 4 | **react-crash-guard 替换 ErrorBoundary** | 周期 9 P2：用 react-crash-guard 替换 ErrorBoundary.jsx；加 async error 捕获 + unhandledrejection 监听 | 升级 client |
| 5 | **cesium-mcp + claude-code-action 深度集成** | 周期 9+ 评估：claude-code-action GitHub Action 接 MiniMax-M3；周期主调度用 M3 跑 PR review | 升级 pr-review.yml |
